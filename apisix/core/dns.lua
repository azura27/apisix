...
local utils = require("apisix.core.dns_utils")
...
local orderValids = {"LAST", "SRV", "A", "CNAME"} -- default order to query
...
local service_suffixes_and_length = {}  -- host with suffix must be a SRV record, and it's length, eg. { .query.consul = 13 }
...
local dns_type_list = { _M.TYPE_LAST, _M.TYPE_SRV, _M.TYPE_A, _M.TYPE_CNAME }
local SECTION_AR  = 3
...
local cachelookup = function(qname, qtype)
  ...
  if cached then
    ...
    if (cached.expire < now) then
      cached.expired = true
      --[[
      log.warn("cache get (stale): ", key, " ", frecord(cached))
      --]]
    else
      --[[
      log.warn("cache get: ", key, " ", frecord(cached))
      --]]
    end
  else
    --[[
  ...
  if not entry.expire then
    ...
    if e1 then
      ...
      for i = 1, #entry do
        ...
        -- set ttl for node type in additional answers
        if record.section == SECTION_AR then
          local names = pl_string.split(record.name, ".")
          if names and names[#names - 2] == "node" then
            record.ttl = nodeTypeTtl
          end
        end
    ...
    elseif entry.errcode == 3 then
      ...
      if (cachelookup(qname, qtype) or empty)[1] then
        -- we still have a stale record with data, so we're not replacing that
        --[[
        log.warn("cache set (skip on empty): ", key, " ", frecord(entry))
        --]]
        return
      end
      ttl = emptyTtl
    ...
    entry.expired = false
    --[[
    log.warn("cache set (new): ", key, " ", frecord(entry))
    --]]

  else
    -- an existing record reinserted (under a shortname for example)
    -- set entry expired to false after refreshing
    ttl = staleTtl
    key = (qtype or e1.type) .. ":" .. (qname or e1.name)
    entry.expire = now + ttl
    entry.expired = false
  end
  -- cache won't be deleted, update when it's stale
  local qnames = pl_string.split(qname or e1.name, ":")
  if qnames and qnames[1] ~= "short" then
    local short_key = (qtype or e1.type) .. ":short:" .. (qname or e1.name)
    dnscache:set(short_key, entry)
  end
...
--  emptyTtl = conf.dns_error_ttl,          -- ttl in seconds for empty responses (e.g. SOA)
...
--  service_suffixes = conf.dns_srv_suffixes, -- domain name with these suffixes requires SRV record
--  noSynchronisation = conf.dns_no_sync,
--  cacheSize = conf.cacheSize,
--  nodeTypeTtl = conf.node_type_ttl
--        })
-- )
local function init(options)

  log.info("(re)configuring dns client")
  local resolv, hosts, err
  options = options or {}
  staleTtl = options.staleTtl or 4
  log.info("staleTtl = ", staleTtl)
  cacheSize = options.cacheSize or 10000  -- default set here to be able to reset the cache
  nodeTypeTtl = options.nodeTypeTtl
  noSynchronisation = options.noSynchronisation
  log.info("noSynchronisation = ", tostring(noSynchronisation))
  ...
  log.info("query order = ", table_concat(order,", "))
  ...
  if ((type(hostsfile) == "string") and (fileexists(hostsfile)) or
    ...
    log.warn("Hosts file not found: ", tostring(hostsfile))
    hosts = {}
  end
  ...
  for name, address in pairs(hosts) do
    ...
    if address.ipv4 then
      ...
      log.info("adding A-record from 'hosts' file: ",name, " = ", address.ipv4)
    end
    if address.ipv6 then
      ...
      log.info("adding AAAA-record from 'hosts' file: ",name, " = ", address.ipv6)
    end
  end
  ...
  if ((type(resolvconffile) == "string") and (fileexists(resolvconffile)) or
    ...
    log.info("Resolv.conf file not found: ", tostring(resolvconffile))
    resolv = {}
  end
  ...
  if #(options.nameservers or {}) == 0 and resolv.nameserver then
    ...
    for _, address in ipairs(resolv.nameserver) do
      ...
      if t == "ipv6" and not options.enable_ipv6 then
        ...
        log.info("skipping IPv6 nameserver ", port and (ip..":"..port) or ip)
      elseif t == "ipv6" and ip:find([[%]], nil, true) then
        -- ipv6 with a scope
        log.info("skipping IPv6 nameserver (scope not supported) ", port and (ip..":"..port) or ip)
      else
        if port then
  ...
  if #options.nameservers == 0 then
    log.warn("Invalid configuration, no valid nameservers found")
  else
    for _, r in ipairs(options.nameservers) do
      log.info("nameserver ", type(r) == "table" and (r[1]..":"..r[2]) or r)
    end
  end

  options.retrans = options.retrans or resolv.options.attempts or 5 -- 5 is openresty default
  log.info("attempts = ", options.retrans)
  ...
  if options.service_suffixes then
    for _, service_suffix in ipairs(options.service_suffixes) do
      service_suffixes_and_length[service_suffix] = #service_suffix
    end
  end
...
local function individualQuery(qname, r_opts, try_list)
  ...
  local result
  result, err = r:tcp_query(qname, r_opts)
  if not result then
...
local function executeQuery(premature, item)
  ...
  if not r then
    ...
    --[[
    log.info("Query executing: ", item.qname, ":", item.r_opts.qtype, " ", fquery(item))
    --]]
    try_status(item.try_list, "querying")
    item.result, item.err = r:tcp_query(item.qname, item.r_opts)
    if item.result then
      --[[
      log.warn("Query answer: ", item.qname, ":", item.r_opts.qtype, " ", fquery(item),
              " ", frecord(item.result))
      --]]
      parseAnswer(item.qname, item.r_opts.qtype, item.result, item.try_list)
      --[[
      log.warn("Query parsed answer: ", item.qname, ":", item.r_opts.qtype, " ", fquery(item),
              " ", frecord(item.result))
      --]]
    else
      log.warn("Query error: ", item.qname, ":", item.r_opts.qtype, " err=", tostring(err))
    end
  end
...
local function lookup(qname, r_opts, dnsCacheOnly, try_list)
  ...
  if entry.expired then
    ...
    log.info("Using expired record of ", qname)
...
local function resolve(qname, r_opts, dnsCacheOnly, try_list)
  ...
  if r_opts then
    ...
    -- set the ADDITIONAL SECTION to TRUE to avoid query node type address
  ...
  if records then
    ...
    if records.expired then
      ...
      if records.errcode == 3 then
        err = "dns server error: " .. records.errcode .. " " .. records.errstr
      end
      return records, err, try_list
    end
  else
...
local function toip(qname, port, dnsCacheOnly, try_list)
  local opts = nil
  for service_suffix, len in pairs(service_suffixes_and_length) do
    if qname:sub(-len) == service_suffix then
      opts = { qtype = _M.TYPE_SRV }
      break
    end
  end

  if not opts then
    local qnames = pl_string.split(qname, ".")
    if qnames and qnames[#qnames - 2] == "node" then
      opts = { qtype = _M.TYPE_A }
    end
  end

  local rec, err
  rec, err, try_list = resolve(qname, opts, dnsCacheOnly, try_list)
  if err then
    return nil, err, try_list
  end
...
local function remove_cache(qname, qtype, try_list)
  local now = time()
  for service_suffix, len in pairs(service_suffixes_and_length) do
    if qname:sub(-len) == service_suffix then
      qtype = _M.TYPE_SRV
  ...
  if qtype then
    local key = qtype..":"..qname
    local short_key = qtype..":short:"..qname
    if dnscache:get(key) or dnscache:get(short_key) then
      dnscache:get(key).expire = now
      dnscache:get(short_key).expire = now
      local try_list = try_add(try_list, qname, qtype, "instance-events")
      local opts = { qtype = qtype }
      asyncQuery(qname, opts, try_list)
      try_status(try_list, "stale")
    end
  else
    -- node type don't need to refresh cache
    for _, type in ipairs(dns_type_list) do
      local key = type..":"..qname
      local short_key = qtype..":short:"..qname
      if dnscache:get(key) or dnscache:get(short_key) then
        dnscache:get(key).expire = now
        dnscache:get(short_key).expire = now
...
local function setup_client()
  ...
  local dns_conf = clone_tab(local_conf.dns)
  if dns_conf.resolver then
    for i, server in ipairs(dns_conf.resolver) do
      local ip, port = server:match("^([^:]+)%:*(%d*)$")
      servers[i] = { ip, tonumber(port) or 53 }
    end
  end

  log.info("dns conf is:", require("cjson").encode(dns_conf))

  local opts = {
    hosts = dns_conf.dns_hostsfile,
    resolvConf = nil,      -- defaults to system resolv.conf
    nameservers = servers, -- provided list or taken from resolv.conf
    retrans = nil,         -- taken from system resolv.conf; attempts
    timeout = nil,         -- taken from system resolv.conf; timeout
    badTtl = dns_conf.dns_error_ttl, -- ttl in seconds for dns error responses (except 3 - name error)
    emptyTtl = dns_conf.dns_not_found_ttl,          -- ttl in seconds for empty and "(3) name error" dns responses
    staleTtl = dns_conf.dns_stale_ttl,   -- ttl in seconds for records once they become stale
    order = dns_conf.dns_order,          -- order of trying record types
    service_suffixes = dns_conf.dns_srv_suffixes, -- domain name with these suffixes requires SRV record
    noSynchronisation = dns_conf.dns_no_sync,
    cacheSize = dns_conf.cacheSize,
    nodeTypeTtl = dns_conf.node_type_ttl
  }
...
_M.remove_cache = remove_cache
_M.setup_client = setup_client
