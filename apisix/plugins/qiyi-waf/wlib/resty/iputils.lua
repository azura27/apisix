...
local function ip_in_range(ip, ip_range)
    local ip_phase = {}
    local ip_phase_range = {}
    
    local i = 0;
    for n in string.gmatch(ip, "([^.]+)") do
        ip_phase[i] = n
        i = i + 1
    end

    i = 0
    for n in string.gmatch(ip_range, "([^.]+)") do
        ip_phase_range[i] = n
        i = i + 1
    end

    for i=0,3 do
        if ip_phase[i] ~= ip_phase_range[i] then
            if string.find(ip_phase_range[i], "-") then
                local range = {}

                local m = 0
                for n in string.gmatch(ip_phase_range[i], "([^-]+)") do
                    range[m] = tonumber(n)
                    m = m + 1
                end

                if tonumber(ip_phase[i]) < range[0] or tonumber(ip_phase[i]) > range[1] then
                    return false
                end
            end
        end
