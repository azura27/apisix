...
local ac          = require "apisix.plugins.qiyi-waf.wlib.waf.load_ac"
local base        = require "apisix.plugins.qiyi-waf.wlib.waf.base"
local dns         = require "apisix.plugins.qiyi-waf.wlib.resty.dns.resolver"
local iputils     = require "apisix.plugins.qiyi-waf.wlib.resty.iputils"
local libinject   = require "apisix.plugins.qiyi-waf.wlib.resty.libinjection"
...
function _M.key_contains(haystack, needle)
	if type(haystack) ~= "table" then
		return false;
	end

	local exists = util.table_has_key(needle, haystack)
...
function _M.ip_match(input, pattern)
	if string.find(pattern, "/") then
		return _M.cidr_match(input, pattern)
	elseif string.find(pattern, "-") then
		return iputils.ip_in_range(input, pattern), input
	else
		return input == pattern, input
	end
end

function _M.bwl_match(input, pattern)
	if type(input) ~= 'table' then
		return false, nil
	else
		local prefix = ""
		if pattern == 'sys_black' then
			prefix = "sysb"
		elseif pattern == 'sys_white' then
			prefix = "sysw"
		elseif pattern == 'biz_black' then
			prefix = input.BUSINESS_NAME .. "b"
		elseif pattern == 'biz_white' then
			prefix = input.BUSINESS_NAME .. "w"
		else
			return false, nil
		end

		return bwl.check(prefix, input)
	end
end
...
_M.lookup = {
	...
	KEY_CONTAINS = function(waf, collection, pattern) return _M.key_contains(waf, collection, pattern) end,
	...
	BWL_MATCH    = function(waf, collection, pattern) return _M.bwl_match(collection, pattern) end,
	IP_MATCH     = function(waf, collection, pattern) return _M.ip_match(collection, pattern) end,
}
